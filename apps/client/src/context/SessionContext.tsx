  ...
  useEffect(() => {
    ...
    if (token) {
      apiClient.auth.getMe()
        ...
        .catch(() => { if (active) { apiClient.clearToken(); setUser(null); } })
        .finally(() => active && setLoading(false));
    } else if (import.meta.env.DEV) {
      apiClient.auth.login('dev-1001', 'DEFAULT')
        .then(({ sessionToken, user }) => { if (!active) return; apiClient.setToken(sessionToken); setUser(user); })
        .catch(() => {})
        .finally(() => active && setLoading(false));
    } else {
      setLoading(false);
