...
# 저장소 내 위키 관련 파일 변경을 감시하여 자동으로 `pnpm run index` 실행.
# launchd 데몬(`com.studyops.wiki-watcher.plist`)이 이 스크립트를 백그라운드로 실행.
#
# 사용:
#   pnpm wiki:watch                     # 포어그라운드 실행 (데몬 없이)
...
#
# 트리거 시: `pnpm run index` (docs/wiki 안에서) 실행.
# dev 서버(`pnpm wiki:dev`)가 떠 있으면 Astro가 자동으로 위키 페이지를 새로고침.
#
set -euo pipefail
...
echo "   명령      : pnpm run index (파일 변경 시)"
echo ""
echo "감시 대상:"
...
# 직접 추가하므로 그때 `pnpm wiki:index` 수동 실행하면 됨 (레거시 파일은 자동 수집됨).
#
